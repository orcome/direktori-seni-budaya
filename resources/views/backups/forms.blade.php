@if (request('action') == 'delete' && Request::has('file_name'))
    <div class="card card-danger">
        <div class="card-header">{{ __('backup.delete') }}</div>
        <div class="card-body">
            <p>{!! __('backup.sure_to_delete_file', ['filename' => request('file_name')]) !!}</p>
        </div>
        <div class="card-footer">
            <a href="{{ route('backups.index') }}" class="btn btn-secondary">{{ __('backup.cancel_delete') }}</a>
...
@if (request('action') == 'restore' && Request::has('file_name'))
    <div class="card card-warning">
        <div class="card-header">{{ __('backup.restore') }}</div>
        <div class="card-body">
            <p>{!! __('backup.sure_to_restore', ['filename' => request('file_name')]) !!}</p>
        </div>
        <div class="card-footer">
            <a href="{{ route('backups.index') }}" class="btn btn-secondary">{{ __('backup.cancel_restore') }}</a>
